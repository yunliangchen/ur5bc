...
class ResNetEncoder(nn.Module):
    ...
    def __call__(
        ...
        # TODO handle when task is a list of maps
        ...
        for i, block_size in enumerate(self.stage_sizes):
            ...
            if self.fuse_ftmaps:
                # use same resnets for task encoder and policy so that HxW matches
        ...
        # only use this option if you know they are the same size
        if self.task_highway:
            x = x + orig_t
...
resnetv1_configs = {
    ...
    "resnetv1-18-bridge-task-x-units": ft.partial(
        ResNetEncoder,
        stage_sizes=(2, 2, 2, 2),
        block_cls=ResNetBlock,
        num_spatial_blocks=8,
