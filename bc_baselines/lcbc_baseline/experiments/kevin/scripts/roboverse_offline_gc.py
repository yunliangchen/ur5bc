...
from jaxrl_m.data.goal_conditioned_buffer import GCReplayBuffer, load_gc_roboverse_data
...
flags.DEFINE_integer("num_eval_batches", int(32), "Number of validation batches.")
...
def main(_):
    ...
    with tf.io.gfile.GFile(
        os.path.join(FLAGS.data_path, "train/metadata.npy"), "rb"
    ) as f:
        action_metadata = np.load(f, allow_pickle=True).item()

    with tf.io.gfile.GFile(
        os.path.join(FLAGS.data_path, "val/eval_goals.npy"), "rb"
    ) as f:
    ...
    eval_env = make_gc_env(
        ...
        save_video_dir=os.path.join(FLAGS.save_dir, "videos"),
    ...
    train_paths = tf.io.gfile.glob(
        tf.io.gfile.join(FLAGS.data_path, "train/*.tfrecord")
    )
    val_paths = tf.io.gfile.glob(tf.io.gfile.join(FLAGS.data_path, "val/*.tfrecord"))
    if kwargs["replay_buffer"]:
        buffer = GCReplayBuffer(
        ...
        load_gc_roboverse_data(
            train_paths, buffer, kwargs["dataset_kwargs"]["use_proportion"]
        )
        train_data_iter = buffer.get_iterator()
    else:
    ...
    kwargs["dataset_kwargs"]["use_proportion"] = 1  # use all data for validation
    ...
    for i in tqdm.tqdm(
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            eval_info = evaluate_gc(
                policy_fn, eval_env, num_episodes=FLAGS.eval_episodes
            )
