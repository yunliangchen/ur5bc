...
def main(_):
    ...
    while True:
        ...
        # ask for sticky gripper num steps 
        global STICKY_GRIPPER_NUM_STEPS
        STICKY_GRIPPER_NUM_STEPS = int(input("Sticky gripper num steps?"))
        ...
        # if FLAGS.initial_eep is not None:
        #     print('Moving to initial gripper position')
        ...
        if FLAGS.high_res:
            ...
            curr_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            video_path = os.path.join(base_path, f"{curr_time}_{policy_name}_sticky_{STICKY_GRIPPER_NUM_STEPS}.gif")
            full_images[0].save(
                video_path,
                format="GIF",
