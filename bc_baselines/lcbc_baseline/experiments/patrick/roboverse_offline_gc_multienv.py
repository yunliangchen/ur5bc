...
def main(_):
    ...
    if FLAGS.debug:
        print("DEBUG MODE: rewriting variables")
        FLAGS.eval_episodes = 1
        FLAGS.num_episodes_per_video = 1
        FLAGS.num_episodes_per_row = 1
        FLAGS.num_train_iters = 2
        FLAGS.num_eval_batches = 1
        FLAGS.batch_size = 2
        FLAGS.log_interval = 1
        FLAGS.eval_interval = 1
        FLAGS.save_interval = 1
        TASKS = TASKS[:2]
        # FLAGS.expl_interval = 1
        # FLAGS.eval_interval = 1
    ...
    for task_name in TASKS:
        with tf.io.gfile.GFile(
            tf.io.gfile.join(FLAGS.data_path, f"val/{task_name}_eval_goals.npy"), "rb"
        ) as f:
        ...
        eval_env = make_gc_env(
            ...
            save_video_dir=tf.io.gfile.join(save_dir, "videos", task_name),
            save_video_prefix="eval",
        )
        eval_envs[task_name] = eval_env

    kwargs = FLAGS.config.model_config.to_dict()

    encoder_def = encoders[kwargs["encoder"]](**kwargs["encoder_kwargs"])

    sample_env_eval = eval_envs[list(eval_envs.keys())[0]]
    unrep_agent = globals()[FLAGS.config.model_constructor](
        seed=FLAGS.seed,
        observations=sample_env_eval.observation_space.sample(),
        goals=sample_env_eval.observation_space.sample(),
        actions=sample_env_eval.action_space.sample(),
    ...
    for i in tqdm.tqdm(
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            for task_name, eval_env in eval_envs.items():
                ...
                for k, v in eval_info.items():
                    wandb_logger.log(
                        {f"evaluation/{task_name}/{k}": np.array(v)}, step=i
                    )
                ...
                if FLAGS.save_video:
                    ...
                    wandb_logger.log(
                        {f"evaluation/{task_name}/video": eval_video}, step=i
                    )
